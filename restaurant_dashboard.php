...
<main id="restaurant-main" class="restaurant-main">
    <header class="restaurant-page-heading">
        <div><h1>Restaurant Overview</h1><p id="restaurant-overview-date">Today’s server-backed restaurant activity</p></div>
    ...
    <section class="restaurant-kpi-grid" data-overview-kpis aria-label="Restaurant performance summary">
        <article class="restaurant-card restaurant-kpi"><i class="fa-solid fa-dollar-sign restaurant-kpi-icon" aria-hidden="true"></i><div><p>Today’s revenue</p><h2 data-kpi="revenue">$0.00</h2><span class="restaurant-positive">Derived from completed server orders</span></div></article>
        <article class="restaurant-card restaurant-kpi"><i class="fa-solid fa-bag-shopping restaurant-kpi-icon" aria-hidden="true"></i><div><p>Orders</p><h2 data-kpi="orders">0</h2><span class="restaurant-positive">Server order activity</span></div></article>
        ...
        <article class="restaurant-card restaurant-kpi"><i class="fa-regular fa-circle-check restaurant-kpi-icon" aria-hidden="true"></i><div><p>Acceptance rate</p><h2 data-kpi="acceptance">—</h2><span class="restaurant-positive">Confirmed from server orders</span></div></article>
    </section>
    <section class="restaurant-overview-grid" aria-label="Restaurant operations">
        <article class="restaurant-card restaurant-chart" data-overview-chart aria-labelledby="revenue-orders-title"><header class="restaurant-card-header"><h2 id="revenue-orders-title">Revenue &amp; orders</h2><a href="restaurant_analytics.php">View analytics</a></header><p class="restaurant-empty" data-chart-summary>Revenue and order activity will appear as server orders are completed.</p><div class="restaurant-chart-bars" aria-hidden="true" data-chart-bars></div><div class="restaurant-chart-labels" aria-hidden="true"><span>12 AM</span><span>6 AM</span><span>12 PM</span><span>6 PM</span><span>Now</span></div></article>
    ...
    <section class="restaurant-card restaurant-low-stock" data-low-stock aria-labelledby="low-stock-title"><i class="fa-solid fa-triangle-exclamation" aria-hidden="true"></i><div><h2 id="low-stock-title" data-low-stock-title>Menu availability</h2><p data-low-stock-copy>Availability is loaded from the server catalog.</p></div><a class="restaurant-primary-action" href="restaurant_menu.php">Review menu</a></section>
</main>
<script>
document.addEventListener('DOMContentLoaded', async function () {
    const ui = window.SavoraRestaurantUI;
    const api = window.SavoraApi;
    if (!ui || !api) return;
    let catalog = {};
    let orders = [];
    try {
        catalog = await api.get('api/catalog.php?scope=restaurant');
        const orderData = await api.get('api/orders.php?pageSize=50');
        orders = Array.isArray(orderData && orderData.orders) ? orderData.orders : [];
    } catch (error) {
        const feedback = document.querySelector('[data-chart-summary]');
        if (feedback) feedback.textContent = error.message || 'Server order data is temporarily unavailable.';
    }
    ...
    set('[data-chart-summary]', `${ui.formatMoney(revenue)} in completed revenue across ${completed.length} completed server order${completed.length === 1 ? '' : 's'}.`);

    const unavailable = (Array.isArray(catalog.items) ? catalog.items : []).filter(item => item.available === false).map(item => item.name || 'Unnamed menu item');
    if (unavailable.length) { set('[data-low-stock-title]', `${unavailable.length} menu item${unavailable.length === 1 ? ' is' : 's are'} unavailable`); set('[data-low-stock-copy]', `${unavailable.join(' and ')} ${unavailable.length === 1 ? 'is' : 'are'} currently unavailable in the server catalog.`); }
    else { set('[data-low-stock-title]', 'Menu availability looks good'); set('[data-low-stock-copy]', 'All tracked menu items are available in the server catalog.'); }
});
</script>
