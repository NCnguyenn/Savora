...
**Architecture:** A focused PHP service owns coordinate validation, Geoapify transport, normalization, and safe errors. The authenticated `api/location.php` endpoint exposes role-owned location snapshots plus idempotent GPS/manual save commands. Migration `016_profile_locations.php` owns the schema. A shared browser module wraps one-shot geolocation and focused API calls; role controllers render server snapshots without browser-authoritative caches.
...
- `database/migrations/016_profile_locations.php`: explicit profile-location schema migration.
- `api/location.php`: authenticated snapshots and location commands.
...
- Create: `database/migrations/016_profile_locations.php`
- Create: `lib/profile_locations.php`
- Create: `api/location.php`
...
test('focused API exposes role-owned location commands', () => {
...
- [ ] **Step 3: Add explicit idempotent schema migration**

Register migration `016_profile_locations.php` in `lib/migrations.php` and use its validated column helper:
...
Require the two location libraries from `api/location.php`. GET returns `location => savora_profile_location($conn, $role, $userId)` for allowed roles. POST accepts only the two focused actions:
...
```bash
git add lib/migrations.php database/migrations/016_profile_locations.php lib/profile_locations.php api/location.php tests/admin_schema_test.php tests/location_api_contract.test.js
