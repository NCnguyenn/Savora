...
function finance_issue_refund(mysqli $conn, int $actorId, int $caseId, float $amount, string $destination, string $reason, int $expectedVersion, string $idempotencyKey): array
{
    ...
    try {
        ...
        $customerId = (int) $case['customer_user_id']; $negative = -$amount; $ledgerReference = 'LED-REF-' . $refundId; $method = (string) $payment['method']; $ledger = $conn->prepare("INSERT INTO ledger_entries(reference_code,order_id,entry_type,party_type,party_id,gross_amount,net_amount,payment_method,status) VALUES(?,?,'refund','customer',?,?,?,?,'completed')"); $ledger->bind_param('siidds', $ledgerReference, $orderId, $customerId, $negative, $negative, $method); $ledger->execute(); $ledger->close();
