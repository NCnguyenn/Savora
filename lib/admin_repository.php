...
function admin_customers_data(mysqli $conn, array $filters): array
{
    $query = mb_substr(trim((string) ($filters['q'] ?? '')), 0, 80);
    $like = '%' . $query . '%';
    $customers = admin_rows($conn, "SELECT u.id, u.full_name, u.username, u.email, u.phone, u.status, u.last_login_at, u.created_at, u.version, p.address AS location_address, p.latitude, p.longitude, p.location_method, p.location_updated_at, COALESCE(p.wallet_balance, 0) AS wallet_balance, COUNT(DISTINCT o.id) AS order_count, COALESCE(SUM(CASE WHEN o.status = 'delivered' THEN o.total ELSE 0 END), 0) AS lifetime_value, MAX(o.placed_at) AS last_order_at, COUNT(DISTINCT c.id) AS open_cases FROM users u LEFT JOIN customer_profiles p ON p.user_id = u.id LEFT JOIN orders o ON o.customer_user_id = u.id LEFT JOIN support_cases c ON c.reporting_user_id = u.id AND c.status NOT IN ('resolved','closed') WHERE u.role = 'customer' AND (? = '' OR u.full_name LIKE ? OR u.email LIKE ?) GROUP BY u.id, u.full_name, u.username, u.email, u.phone, u.status, u.last_login_at, u.created_at, u.version, p.address, p.latitude, p.longitude, p.location_method, p.location_updated_at, p.wallet_balance ORDER BY u.created_at DESC", 'sss', [$query, $like, $like]);
...
function admin_restaurant_partner_data(mysqli $conn, array $filters): array
{
    ...
    return [
        ...
        'restaurants' => admin_rows($conn, 'SELECT r.id, r.name, r.cuisine, r.city, r.status, r.accepting_orders, r.rating, r.cancellation_rate, r.payout_status, r.address AS location_address, r.latitude, r.longitude, r.location_method, r.location_updated_at, u.full_name AS owner_name, u.email FROM restaurants r JOIN users u ON u.id = r.owner_user_id ORDER BY r.created_at DESC'),
...
function admin_driver_partner_data(mysqli $conn, array $filters): array
{
    ...
    return [
        ...
        'drivers' => admin_rows($conn, 'SELECT d.id, u.id AS user_id, u.full_name, u.email, u.status, d.vehicle_type, d.license_plate, d.service_area, d.address AS location_address, d.latitude, d.longitude, d.location_method, d.location_updated_at, d.eligibility_status, d.availability_status, d.rating, d.acceptance_rate, d.completion_rate FROM driver_profiles d JOIN users u ON u.id = d.user_id ORDER BY d.created_at DESC'),
